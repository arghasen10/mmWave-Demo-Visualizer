...
(function()
{
    ...
    {
        var step = -1;
        while (this.first !== this.last && step < 0)
        {
     ...
     * gc.console.log() method for debug purposes before forwarding the packet to the next 
     ...
     * to gc.console.log().  The derived implementation should override this method to implement the 
     ...
     */
    gc.databind.AbstractMessageBasedCodec.prototype.isConnected = function()
    {
     ...
     * register operation. 
     *
     * @params startRegInfo register information about the starting register for the multiple register read
     * @params count the number of registers to read.    
     * @returns a promise that will resove with an array of register values read from the target. 
     */
    gc.databind.AbstractMessageBasedCodec.prototype.doMultiRegisterRead = function(startRegInfo, count)
    {
        var promises = [];
        var info = Object.create(startRegInfo);
        for(var i = 0; i < count; i++)
        {
            promises.push(this.readValue(info));
            info.addr++;
        }
